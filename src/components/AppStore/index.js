...
class AppStore extends Component {
  ...
  render() {
    ...
    const displayList = appsList.filter(
      eachItem => eachItem.category === selectedTabId,
    )

    const newAppDisplayList = displayList.filter(eachItem =>
    ...
    return (
      <div className="app-bg-container">
        ...
        <ul className="app-item-container">
          {newAppDisplayList.map(eachItem => (
            <li className="app-item" key={eachItem.appId}>
              <AppItem appObj={eachItem} />
            </li>
          ))}
        </ul>
      </div>
